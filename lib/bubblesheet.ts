...
export const templateConfigs = {
  5: { name: '5 Questions', columns: 1, questionsPerColumn: 5, description: 'Quick quiz' },
  10: { name: '10 Questions', columns: 1, questionsPerColumn: 10, description: 'Short quiz' },
  15: { name: '15 Questions', columns: 1, questionsPerColumn: 15, description: 'Mini test' },
  20: { name: '20 Questions', columns: 2, questionsPerColumn: 10, description: 'Standard quiz' },
  25: { name: '25 Questions', columns: 2, questionsPerColumn: 13, description: 'Short test' },
  50: { name: '50 Questions', columns: 2, questionsPerColumn: 25, description: 'Standard test' },
  100: { name: '100 Questions', columns: 4, questionsPerColumn: 25, description: 'Extended test' }
};
...
export const generateBubbleSheetPDF = (questions: number): jsPDF => {
  ...
  const columnWidth = (pageWidth - 2 * margin - (config.columns - 1) * 20) / config.columns;
  ...
  // Instructions
  doc.setFontSize(8);
  doc.text('Fill bubbles completely with a #2 pencil. Make no stray marks.', pageWidth / 2, 50, { align: 'center' });
  ...
  doc.text('Student ID: __________________', pageWidth - margin - 80, 67);
  doc.text('Date: _______________', pageWidth / 2, 67);
  ...
  for (let col = 0; col < config.columns; col++) {
    const startX = margin + col * (columnWidth + 20);
  ...
  for (let q = 1; q <= questions; q++) {
    ...
    const startX = margin + column * (columnWidth + 20);
  ...
  doc.text('Generated by SnapGrade • Ensure good lighting and clear marks for accurate scanning', 
...
`;
  ...
  for (let q = 1; q <= questions; q++) {
    ...
    // Question number
    svg += `<text x="${x - 20}" y="${y + 5}" font-family="Arial, sans-serif" font-size="10" text-anchor="end">${q}.</text>`;
    
    // Answer bubbles A, B, C, D
    ['A', 'B', 'C', 'D'].forEach((letter, index) => {
      const bubbleX = x + (index * bubbleSpacing);
      svg += `
        <circle cx="${bubbleX}" cy="${y}" r="${bubbleSize/2}" fill="none" stroke="#000" stroke-width="1"/>
        <text x="${bubbleX}" y="${y - 15}" text-anchor="middle" font-family="Arial, sans-serif" font-size="8">${letter}</text>
      `;
    });
  }
